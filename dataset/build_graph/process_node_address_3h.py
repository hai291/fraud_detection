...
def process_address_graph_with_snapshot(type_, class_name, phish_set, address_to_id, next_addr_id):
    ...
    first_index = 0
    start_time = df.loc[0, 'block_timestamp']
    check_3h = []
    check_2w = []
    ...
    while first_index < len(df):
        ...
        for row in df.iloc[first_index:].itertuples(index=True):
            if row.block_timestamp - start_time >= 10800:  
                ...
                slice_df = df.iloc[first_index:last_index]

                for tx in slice_df.itertuples(index=False):
                ...
                check_3h.append(last_timestamp)
                found_3h = True
                break
        ...
        for row in df.iloc[first_index:].itertuples(index=True):
            if row.block_timestamp - start_time >= 1209600:  
                ...
                check_2w.append(start_time)
                skip_2w = True
                print(f'Skipped 2 weeks after snapshot.')
                break
...
if __name__ == "__main__":
    ...
    for type_, cls in [('in', 'phisher'), ('out', 'phisher'), ('in', 'normal'), ('out', 'normal')]:
        edges, out_c, in_c, out_v, in_v, ts, address_to_id, next_addr_id = process_address_graph_with_snapshot(
    ...
    build_graph_from_addresses(
        ...
        '/home/hainguyen/fraud-detection-credit/dataset/processed/b4e_address_graph_3h'
    )
